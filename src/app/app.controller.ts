...
export class AppController {
  ...
  public $modalOpen(modalState: IModalState): void {
    ...
    this.$_modalStates.update((v) => ({
      ...(v || {}),
      [String(key)]: modalState,
    }));
  }
  ...
  public $modalClose(key?: number): void {
    if (
      !key ||
      !this.$_modalStates() ||
      Object.keys(this.$_modalStates() || {}).length === 1 ||
      !this.$_modalStates()?.[key]
    ) {
      // close All
  ...
  public $modalList = computed(() =>
    Object.keys(this.$_modalStates() || [])
      .sort()
      .map((k) => this.$_modalStates()![Number(k)])
  );
