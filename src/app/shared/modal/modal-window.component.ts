...
import {
  Component, ComponentRef, effect, ElementRef, inject, Injector, input, model, OnDestroy, signal, Signal, untracked,
  ViewChild, ViewContainerRef, computed, ModelSignal, TemplateRef, ChangeDetectionStrategy
} from '@angular/core'
import { NgClass } from '@angular/common'

import { ModalComponent } from './modal.component'
import { AppState } from 'src/app/state/app.state';
import { EntityPathComponent } from 'src/app/shared/components/entity-path/entity-path.component';
import { FormComponent } from 'src/app/shared/form/form.component'
import { IInputCheckboxConfig, InputCheckboxComponent } from 'src/app/shared/form/inputs/input-checkbox/input-checkbox.component'
import { ButtonComponent } from 'src/app/shared/_ui/button/button.component'
...
import { IModalWindowConfig, IModalState, ModalWindowType, ModalButtonKey, ModalButton } from './modal.types'
import { Form } from '../form/form.namespace';

// Реэкспорты для обратной совместимости
export { IModalWindowConfig, IModalState, ModalWindowType, ModalButtonKey, ModalButton } from './modal.types'
...
const MODAL_WINDOW_BUTTONS: Record<'cancel' | 'delete' | 'save', Omit<ModalButton, 'key'>> = {
  cancel: { class: 'secondary', title: 'buttons.cancel', type: 'secondary-stroked',  order: 20 },
  delete: { class: 'delete',    title: 'buttons.delete', type: 'danger',   order: 30 },
  save:   { class: 'primary',   title: 'buttons.save',   type: 'primary',   order: 30 },
}
...
@Component({
  ...
  imports: [ NgClass, InputCheckboxComponent, EntityPathComponent, ButtonComponent, FormComponent, IconComponent,
    LanguagePipe ],
...
export class ModalWindowComponent implements OnDestroy {
  ...
  protected onKeydown( e: KeyboardEvent ): void {
    ...
    if( !this.config().noCloseOnEsc && key === 'Escape' && this.appCtrl.$modalLast().key === this.state().key ) this.close()
  }

  protected onClick( e: MouseEvent ): void {
    if( !this.config().noCloseOnClickOutside && e?.target === this.ref.nativeElement && e.type ) this.close()
  }
  ...
  protected type        = computed<ModalWindowType>( () => this.config().type ?? 'edit' )
  // protected form        = computed( ()=> !this.formItems?.() ? null : { config: this.formItems!() } )

  protected modalStyles = computed( ()=>{
    const effectivePaddingHorizontal = this.config().paddingHorizontal ?? this.config().padding ?? 32
    const effectivePaddingVertical = this.config().paddingVertical ?? this.config().padding ?? 32
    ...
    if( this.config().minHeight ) h.push( `min-height: ${this.config().minHeight};` )
    if( this.config().maxHeight ) h.push( `max-height: ${this.config().maxHeight};` )
  ...
  protected readonly bgColor     = computed( () => this.config().bgColor || 'var(--theme-background-bg-light)' )
  protected readonly yScrollType = computed( () => this.config().overrideYScroll ? 'hidden' : 'auto' )
  protected readonly isSafeHtml  = computed<boolean>( ()=> this.note() != null && typeof this.note() !== 'string' )
  ...
  public footerCheck = model( false )
  public footerSwitch = model( false )
  public closeHandler: IModalState<any>[ 'closeHandler' ]

  public footerCheckConfig = computed<IInputCheckboxConfig>( () => ({
    type: 'check',
    text: this.config().footerCheck,
    wrapperClass: null,
    ...this.config().footerCheckExtraConfig,
  }))
    ...
    switch( key ){
      ...
      case 'delete':
        if( this.formInputs() ){
          ...
          if( this.formIsValid() !== true ){ console.warn( errors ) ; return }
        }
        try{
  ...
  get modalComponent(): ModalComponent { return this.componentRef.instance }
  ...
  constructor(){
    effect( ()=>{
      ...
      untracked( ()=>{
        ...
        this.componentRef = this.container.createComponent( state.component!, { injector: this.injector } )
        ...
        const configValue = this.config().footerCheckExtraConfig?.value
        const footerCheckInitialValue = typeof configValue === 'boolean' ? configValue : configValue?.() ?? false
        this.footerCheck.set( footerCheckInitialValue )
        component.footerCheck = this.footerCheck
        component.footerSwitch = this.footerSwitch
  ...
  protected onModalBodyClick( event: MouseEvent ) {
    ...
    if( isOnTeamPage && !clickedElement.closest( '.activity-log__content' ) )  event.stopPropagation()
  }
