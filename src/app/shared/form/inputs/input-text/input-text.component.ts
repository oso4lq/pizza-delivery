...
@Component({
  ...
  imports: [NgClass, InputsWrapperComponent, IconComponent],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class InputTextComponent
  extends InputComponent<IInputTextConfig>
  implements AfterViewInit
{
  ...
  readonly maxChars = computed<number | undefined>(() => {
    ...
    if (cfg.limitChars && cfg.validate?.max)
      return String(cfg.validate?.max).length; // Для числовых значений
  ...
  override configSetup(config: IInputTextConfig): IInputTextConfig {
    ...
    if (updConfig.validate && !updConfig.validate.type)
      updConfig.validate!.type = 'string';
    return updConfig;
  }
  ...
  protected onKeyDown(event?: KeyboardEvent): boolean | undefined {
    ...
    let isValid =
      !this.config()?.mask || this.config()?.mask!.test(String(event.key));
  ...
  onInput(event?: Event): void {
    ...
    if (this.config().transformInput && inputElement) {
      const transformed = this.config().transformInput!(
        inputElement.value,
