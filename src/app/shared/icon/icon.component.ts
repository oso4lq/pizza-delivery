...
export class IconComponent {
  ...
  name = input( undefined, {
    transform: ( name?: IconName ) => {
      if( !name ) { this.icon.set(null) ; return name }
      this.isIcon.set( true )

      const symbol = this.spriteSrv.getSymbol( name )
      if( !symbol ) throw new Exception( 'Core', `Invalid icon name: ${ name }` )
      const vb = symbol.getAttribute('viewBox')
      this.viewBox.set( vb ? vb : `0 0 ${ this.$width() } ${ this.$height() }` )
      this.symbolContent.set( this.sanitizer.bypassSecurityTrustHtml( symbol.innerHTML ) )
  ...
  src = input( undefined, {
    transform: ( src?: string ) => {
      ...
      if( !src ) { this.icon.set(null) ; return src }
      ...
      if( isIcon ){
        ...
        if( !symbol ) throw new Exception( 'Core', `Invalid icon name: ${ src }` )
        const vb = symbol.getAttribute( 'viewBox' )
        this.viewBox.set(vb ? vb : `0 0 ${this.$width()} ${ this.$height() }`)
        this.symbolContent.set( this.sanitizer.bypassSecurityTrustHtml( symbol.innerHTML ) )
      }
  ...
  width = input( undefined, { transform: ( w?: number )=>{ this.$width.set( w ?? this.defaultIconSize ) ; return w } })
  height = input( undefined, { transform: ( h?: number )=>{ this.$height.set( h ?? this.defaultIconSize ) ; return h } })
  size = input( undefined, { transform: ( s?: number )=>{ this.$height.set( s ?? this.defaultIconSize ) ; this.$width.set( s ?? this.defaultIconSize ) ; return s } })
  
  // ключ внутри COLORS (текущей темы)  
  color = input( undefined, { transform: ( color?: ColorKey ) => { this.$colorHex.set( COLORS[ color ?? 'secondary' ] ) ; return color } }) 

  // прямое указание цвета
  colorHex = input( undefined, { transform: ( colorHex?: string ) => { this.$colorHex.set( colorHex ?? 'currentColor' ) ; return colorHex } }) 

  tp = input<any>() // tooltip

  clickable = input<boolean>( false ) // некликабельность
  iconClick = output<void>() // emitter
  handleClick() { if( !this.clickable() ) this.iconClick.emit() }
